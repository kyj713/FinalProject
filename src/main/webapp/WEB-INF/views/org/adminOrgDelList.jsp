...
// 부서 삭제목록 데이터 불러오기
function getDeletedDeptData(){
    var xhr = new XMLHttpRequest();
    xhr.open("get", "/adminOrg/getDeletedDeptData",true);
...
// 사원 삭제목록 데이터 불러오기
function getDeletedEmpData() {
	var xhr = new XMLHttpRequest();
	xhr.open("get", "/adminOrg/getDeletedEmpData", true);
	xhr.onreadystatechange = function() {
		if(xhr.readyState == 4 && xhr.status == 200) {
			let rslt = JSON.parse(xhr.responseText);
			gridApi.setGridOption('rowData', rslt);
...
function hardDeleteDept() {
    ...
    Swal.fire({
    	...
    	if(result.isConfirmed) {
			var xhr = new XMLHttpRequest();
			xhr.open("delete", "/adminOrg/hardDeleteDept", true);
			...
			xhr.send(JSON.stringify(deptCodes));	// send는 한번만!
    	}
    });
}

// 부서 복구
function recoveryDept() {
	let selectedRows = gridApi.getSelectedRows();
    let deptCodes = selectedRows.map(row => row.deptCode);
    ...
    Swal.fire({
    	...
    	if(result.isConfirmed) {
		    var xhr = new XMLHttpRequest();
		    xhr.open("put", "/adminOrg/recoveryDept", true);
		    xhr.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
		    xhr.setRequestHeader(header, token); // csrf 토큰을 헤더에 추가
		    xhr.onreadystatechange = function() {
		        if(xhr.readyState == 4 && xhr.status == 200) {
		        	Swal.fire({
		                icon: 'success',
		                title: '성공',
		                text: '부서 복구를 성공적으로 완료하였습니다.'
		            });
		            getDeletedDeptData();
		        }
		    }
		    xhr.send(JSON.stringify(deptCodes));
    	}
    });
}
