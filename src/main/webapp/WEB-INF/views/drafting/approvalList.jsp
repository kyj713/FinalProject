                ...
                <div class="d-md-flex gap-4">
                    ...
                    <div class="w-md-175px w-xl-200px flex-shrink-0 mb-3">
                        ...
                        <div class="list-group list-group-borderless gap-1">
                            ...
                            <a href="/drafting/approvalList" class="list-group-item list-group-item-action active">
                    ...
                    <div class="flex-fill min-w-0">
                        <div class="card-body">
                            ...
                            <div class="table-responsive">
                           		<h5 class="mb-1 fw-bold" style="font-size: 20px;">결재 문서함
		                            ...
		                            <div id="recentToSignInfo" class="recent-info" style="display: none;">
									    결재자로 등록된 기안문서를 최근 등록 순서대로 표시합니다.
									</div>
                                <table class="table table-striped align-middle">
                                    ...
                                    <tbody>
                                    	<c:set value="${pagingVO.dataList }" var="toApprvList"/>
                                        <c:choose>
                                        	<c:when test="${empty toApprvList }">
                                        		<tr>
                                        	...
                                        	<c:otherwise>
                                        		<c:forEach items="${toApprvList }" var="toApprv">
                                        			<tr>
                                        				<td>${toApprv.drftNo }</td>
                                        				<td>
                                        					<a class="h6 btn-link text-decoration-underline text-truncate mb-0" href="/drafting/detail?drftNo=${toApprv.drftNo }">
                                        						${toApprv.drftTitle }
                                        					</a>
                                        				</td>
                                        				<td>
                                        					<span class="text-nowrap text-muted">${toApprv.drftDate }</span>
                                        				</td>
                                        				<td>${toApprv.drftFormName }</td>
                                        				<td>${toApprv.drftWriterName } ${toApprv.drftWriterPositionName }</td>
                                        				<td>${toApprv.drftWriterDeptName }</td>
			                                            <td class="h5" style="width: 10%;">
	                                        				<c:if test="${toApprv.drftStatus eq '진행중'}">
				                                               	<div class="d-block badge bg-success">${toApprv.drftStatus }</div>
	                                        				</c:if>
	                                        				<c:if test="${toApprv.drftStatus eq '승인'}">
				                                                <div class="d-block badge bg-light text-dark">${toApprv.drftStatus }</div>
	                                        				</c:if>
	                                        				<c:if test="${toApprv.drftStatus eq '반려'}">
				                                                <div class="d-block badge bg-danger">${toApprv.drftStatus }</div>
	                                        				</c:if>
                                        				</td>
                                        			</tr>
                                        		</c:forEach>
