	...
	<div class="content__wrap">
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb mb-0">
				...
				<li class="breadcrumb-item"><a href="./forms.html">공지사항</a></li>
				<li class="breadcrumb-item active" aria-current="page">공지글 등록하기</li>
			</ol>
		</nav>
	</div>
</div>
...
<div class="content__boxed">
	<div class="content__wrap">
		...
		<form action="/notice/insert" method="post" id="insertForm"
			...
			<input type="hidden" value="${board.boardCode } == 1">
			<c:if test="${status eq 'u' }">
...
$(function() {
	...
	$(".fileDelete").on("click", function(){
		...
		insertBtn.on("click", function() {
			...
			if ($(this).val() == "수정") {
				insertForm.attr("action", "/notice/noticeUpdate");
			}
	
			insertForm.submit();
			if($(this).val() == "등록"){
				webSocket.send("새로운 공지사항이 게시되었습니다!");
			}
		});
		    ...
		    $.ajax({
		        ...
		        success: function(data) {
		            ...
		            if (checkImageType(data)) {
		                str += "<div>";
		                str += "  <a href='/notice/displayFile?fileName=" + data + "'>";
		                str += "    <img src='/notice/displayFile?fileName=" + getThumbnailName(data) + "'/>";
		                str += "  </a>";
		                str += "  <span>X</span>";
		                str += "</div>";
		            } else {
		                str += "<div>";
		                str += "  <a href='/notice/displayFile?fileName=" + data + "'>" + getOriginalName(data) + "</a>";
		                str += "  <span>X</span>";
		                str += "</div>";
		            }
