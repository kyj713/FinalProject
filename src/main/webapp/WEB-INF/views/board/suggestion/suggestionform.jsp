	...
	<div class="content__wrap">
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb mb-0">
				...
				<li class="breadcrumb-item"><a href="./forms.html">건의게시판</a></li>
				<li class="breadcrumb-item active" aria-current="page">건의게시판 등록하기</li>
			</ol>
		</nav>
	</div>
</div>
...
<div class="content__boxed">
	<div class="content__wrap">
		...
		<h1>${name }</h1>
		<hr />
		<form action="/suggestion/suggestioninsert" method="post" id="insertForm"
			enctype="multipart/form-data">
			<input type="hidden" value="${board.boardCode } == 3">
			<c:if test="${status eq 'u' }">
...
//     });
	...
	$(".fileDelete").on("click", function(){
		...
		insertBtn.on("click", function() {
			...
			if ($(this).val() == "수정") {
				insertForm.attr("action", "/suggestion/update");
			}
			insertForm.submit();
		});
		...
		$("#testingBtn").on("click", function(){
			$("#boardTitle").val("프로젝트가 너무 늦어요 ㅠㅠ.");
			CKEDITOR.instances.editor.setData("프로젝트 승인해주세요!");
		});
		    ...
		    $.ajax({
		        ...
		        success: function(data) {
		            ...
		            if (checkImageType(data)) {
		                str += "<div>";
		                str += "  <a href='/board/displayFile?fileName=" + data + "'>";
		                str += "    <img src='/board/displayFile?fileName=" + getThumbnailName(data) + "'/>";
		                str += "  </a>";
		                str += "  <span>X</span>";
		                str += "</div>";
		            } else {
		                str += "<div>";
		                str += "  <a href='/board/displayFile?fileName=" + data + "'>" + getOriginalName(data) + "</a>";
		                str += "  <span>X</span>";
		                str += "</div>";
		            }
