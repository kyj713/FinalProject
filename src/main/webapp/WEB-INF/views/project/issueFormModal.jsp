	...
	<div class="card h-100 p-3 overflow-scroll scrollable-content" id="issueFormModalCont">
		...
		<div class="card-body">
			...
			<form id="issueForm" class="row g-3" method="post" action="/project/issue/register" enctype="multipart/form-data">
					...
					<label for="issueType" class="form-label">유형<span
						...
						<c:forEach var="map" items="${commonCodes }">
							<c:if test="${map['COMMON_CODE_GROUP_ID'] == '901'}">
								<option value="${map['COMMON_CODE'] }">${map['COMMON_CODE_NAME'] }</option>
							</c:if>
						</c:forEach>
					</select>
				</div>
				...
				<div class="col-md-6">
					<label for="issueStatus" class="form-label">상태<span
						class="text-danger">*</span></label> <select id="issueStatus"
						name="issueStatus" class="form-select" required>
						<c:forEach var="map" items="${commonCodes }">
