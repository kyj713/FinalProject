	...
	<div class="content__wrap">
		...
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb mb-0">
				...
				<li class="breadcrumb-item active" aria-current="page">휴가/출장현황</li>
			</ol>
		</nav>
		<!-- Breadcrumb 끝-->
				...
				<div class="d-md-flex" style="height: 740px;">
					...
					<div class="flex-fill d-flex justify-content-center">
						...
						<div style="width: 55%;">
							...
							<div class="card-body mb-5" style="height: 40%;">
							
								<h5 class="card-title fw-bold">휴가 현황</h5>
								...
								<div class="table-responsive">
									<table class="table table-hover">
										<thead>
											<tr>
												<th class="rest-date text-center">기안번호</th>
												<th class="rest-type text-center">제 목</th>
												<th class="rest-approval text-center">승인내역</th>
											</tr>
										</thead>
										<tbody>
											
											<c:set value="${pagingVORest.dataList }" var="restList" />	
											
											<c:choose>										
												<c:when test="${empty restList }">
													<tr class="text-center">
														<td colspan="3">휴가신청서 승인 내역이 없습니다.</td>
													</tr>
												</c:when>
												<c:otherwise>
													<c:forEach items="${restList}" var="restList">
														<tr>
															<td class="rest-date-txt text-center">${restList.drftNo}</td>
															<td class="rest-type-txt text-center">${restList.drftTitle}</td>
															<td class="rest-approval-txt text-center"><a
																href="/drafting/detail?drftNo=${restList.drftNo}" class="text-danger">승인내역확인</a></td>
								...
								<div class="card-footer clearfix d-flex justify-content-center" id="pagingArea">${pagingVORest.pagingHTML }</div>
							</div>


							<div class="card-body mt-5" style="height: 40%;">

								<h5 class="card-title fw-bold">출장 현황</h5>
								...
								<div class="table-responsive">
									<table class="table table-hover">
										...
										<tbody>
											...
											<c:set value="${pagingVOTrip.dataList}" var="tripList" />	
											
											<c:choose>										
												<c:when test="${empty tripList }">
													<tr class="text-center">
														<td colspan="3">출장신청서 승인 내역이 없습니다.</td>
													</tr>
												</c:when>
												<c:otherwise>
													<c:forEach items="${tripList}" var="tripList">
														<tr>
															<td class="rest-date-txt text-center">${tripList.drftNo}</td>
															<td class="rest-type-txt text-center">${tripList.drftTitle}</td>
															<td class="rest-approval-txt text-center"><a
																href="/drafting/detail?drftNo=${tripList.drftNo}" class="text-danger">승인내역확인</a></td>
								...
								<div class="card-footer clearfix d-flex justify-content-center" id="pagingAreaTrip">${pagingVOTrip.pagingHTML }</div>
					...
					<form action="" method="get" id="pagingForm">
						<input type="hidden" name="page" value="${pagingVORest.currentPage }" id="page"/>
						<input type="hidden" name="pageTrip" value="${pagingVOTrip.currentPage }" id="pageTrip"/>
					</form>
...
<script>
	$(function() {
		...
		var pagingAreaTrip = $("#pagingAreaTrip");

		pagingArea.on("click", "a", function(event) {
			event.preventDefault(); //a태그의 이벤트를 block
			var pageNo = $(this).data("page");
			pagingForm.find("#page").val(pageNo);
			pagingForm.submit();
		}); //pagingArea 클릭이벤트 끝
		
		pagingAreaTrip.on("click", "a", function(event) {
			event.preventDefault(); //a태그의 이벤트를 block
			var pageNo = $(this).data("page");
			pagingForm.find("#pageTrip").val(pageNo);
			pagingForm.submit();
		}); //pagingAreaTrip 클릭이벤트 끝
